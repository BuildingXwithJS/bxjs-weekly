## Articles & News:

- [Rendering on the Web](https://developers.google.com/web/updates/2019/02/rendering-on-the-web)
- [Building a modern carousel with CSS scroll snap, smooth scrolling, and pinch-zoom](https://nolanlawson.com/2019/02/10/building-a-modern-carousel-with-css-scroll-snap-smooth-scrolling-and-pinch-zoom/)
- [Announcing Open Web Components](https://dev.to/thepassle/announcing-open-web-components-5h7)
- [Using TypeScript with React](https://alligator.io/react/typescript-with-react/)
- [This benchmark is indeed flawed (Dan Abramov on benchmark for React hooks vs HoCs)](https://medium.com/@dan_abramov/this-benchmark-is-indeed-flawed-c3d6b5b6f97f)
- [Performance vs Readability](https://blog.usejournal.com/performance-vs-readability-2e9332730790)
- [React Hooks in TypeScript](https://medium.com/@jrwebdev/react-hooks-in-typescript-88fce7001d0d)
- [Replacing a hot path in your app's JavaScript with WebAssembly](https://developers.google.com/web/updates/2019/02/hotpath-with-wasm) and [HN discussion](https://news.ycombinator.com/item?id=19166233)
- [DOM performance case study](https://areknawo.com/dom-performance-case-study/)
- [Instrumenting HTTP requests in node](https://medium.com/@tlivings/instrumenting-http-requests-in-node-5bf48c10f1c0)
- [Resource Prioritization – Getting the Browser to Help You](https://developers.google.com/web/fundamentals/performance/resource-prioritization)
- [Experimenting with Bluetooth in JavaScript apps on the web, in hybrid apps and React Native](https://www.voorhoede.nl/en/blog/bluetooth-anywhere/)
- [Simulating blobs of fluid](https://peeke.nl/simulating-blobs-of-fluid)
- [The (new) React lifecycle methods in plain, approachable language](https://blog.logrocket.com/the-new-react-lifecycle-methods-in-plain-approachable-language-61a2105859f3)
- [Outstated - simple hooks-based state management for React](https://dev.to/yamalight/outstated---simple-hooks-based-state-management-for-react-1ji1)
- [Adblockers Performance Study](https://dev.to/remusao/adblockers-performance-study-5b47)
- [Funding ESLint’s Future](https://eslint.org/blog/2019/02/funding-eslint-future)

## Tips, tricks & bit-sized awesomeness:

- [On Captain Marvel website](https://twitter.com/backlon/status/1094037656072114176?s=09)
- [What react hooks can do for you?](https://www.reddit.com/r/reactjs/comments/ap2cp6/what_react_hooks_can_do_for_you/)
- [Promise.allSettled reaches stage 2](https://jason-williams.co.uk/promise-allsettled-reaches-stage-2/)
- [Google Docs now has API](https://developers.google.com/docs/api/how-tos/overview)
- [What's the longest keyword sequence in Javascript?](https://gist.github.com/lhorie/c0d9fd9b2aa215f4984f3ce1c8fd01bf)
- [Bootstrap v5 is getting rid of jQuery](https://github.com/twbs/bootstrap/pull/23586)
- [Early demo of the new React DevTools](https://www.reddit.com/r/reactjs/comments/aq1g6w/early_demo_of_the_new_react_devtools/)
- [Inspectable complex hook values landed in DevTools rewrite](https://twitter.com/brian_d_vaughn/status/1096286056905752576?s=12)
- [PSA: `koa-router` was just transferred to a relatively unknown user on GitHub and the package name was apparently sold](https://github.com/nodejs/package-maintenance/issues/77#issuecomment-463356844)

## Releases:

- [Next.js 8](https://nextjs.org/blog/next-8)
- [windows95 v2.0](https://github.com/felixrieseberg/windows95/releases/tag/v2.0.0)
- [react-native v0.59-rc](https://github.com/facebook/react-native/releases/tag/v0.59.0-rc.1)
- [Riot Web 1.0](https://medium.com/@RiotChat/the-big-1-0-68fa7c6050be)

## Libs & demos:

- [react-hooks-easy-redux](https://github.com/dai-shi/react-hooks-easy-redux)
- [Orca](https://github.com/hundredrabbits/Orca)
- [WasmRoslyn](https://kjpou1.github.io/WasmRoslyn/)
- [instant.page](https://instant.page/)
- [swc](https://github.com/swc-project/swc)
- [react-hooks-async](https://github.com/dai-shi/react-hooks-async)
- [muster](https://github.com/dwstech/muster)
- [nextron](https://github.com/saltyshiomix/nextron)
- [linaria](https://linaria.now.sh/)
- [react.explore-tech.org](https://react.explore-tech.org/)
- [shards-dashboard-react](https://github.com/DesignRevision/shards-dashboard-react)
- [openapi-directory-js](https://github.com/httptoolkit/openapi-directory-js)
- [admin-bro](https://github.com/SoftwareBrothers/admin-bro)
- [redactyl.js](https://github.com/rcjpisani/redactyl.js)
- [routr](https://github.com/fonoster/routr)
- [leon](https://github.com/leon-ai/leon)
- [Motrix](https://github.com/agalwood/Motrix)
- [deskgap](https://deskgap.com/)
- [deploy-to-kube](https://github.com/kubesail/deploy-to-kube)
- [smoke](https://github.com/sinedied/smoke/)
- [split.js](https://split.js.org/)
- [third-party-web](https://github.com/patrickhulce/third-party-web)

## Interesting & silly stuff:

- [🚀 Introducing 🎄ChristmaSSier🎄— an opinionated CSS formatter](https://twitter.com/sompylasar/status/1093750449604243457?s=09)
- [Audio AI: isolating vocals from stereo music using Convolutional Neural Networks](https://towardsdatascience.com/audio-ai-isolating-vocals-from-stereo-music-using-convolutional-neural-networks-210532383785)
- [HTML is 26 years old. CSS is 22. JS is 23.](https://twitter.com/WesleydeSouza/status/1096534562920898562?s=09)
