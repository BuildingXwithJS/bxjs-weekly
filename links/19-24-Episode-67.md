## Getting started:

- [Build a GraphQL + React app with TypeScript](https://blog.logrocket.com/build-a-graphql-react-app-with-typescript/)
- [What is an Abstract Syntax Tree](https://blog.bitsrc.io/what-is-an-abstract-syntax-tree-7502b71bde27)
- [Interactive Charts with D3.js](https://wattenberger.netlify.com/)
- [Create a Custom useFetch() React Hook](https://scotch.io/tutorials/create-a-custom-usefetch-react-hook)
- [How to Use the Web Share API](https://css-tricks.com/how-to-use-the-web-share-api/)
- [When should you be using Web Workers?](https://dassur.ma/things/when-workers/)
- [Error handling - The missing piece of your node.js architecture](https://softwareontheroad.com/error-handling-nodejs/)
- [How to do scroll-linked animations the right way](https://blog.logrocket.com/how-to-do-scroll-linked-animations/)
- [Why ['1', '7', '11'].map(parseInt) returns [1, NaN, 3] in Javascript](https://medium.com/dailyjs/parseint-mystery-7c4368ef7b21)
- [CSRF in Action 🎭](https://smellycode.com/csrf-in-action/)

## Articles & News:

- [The problem with web components](https://adamsilver.io/articles/the-problem-with-web-components/)
- [Micro Frontends](https://martinfowler.com/articles/micro-frontends.html)
- [You (probably) don’t need ReCAPTCHA](https://kevv.net/you-probably-dont-need-recaptcha/)
- [Ask HN: Old guys, what do you miss about development in the 70s, 80s, 90s, 00s?](https://news.ycombinator.com/item?id=20161326)
- [Chromium and the browser monoculture problem](https://dev.to/kenbellows/chromium-and-the-browser-monoculture-problem-420n)
- [The reduce ({...spread}) anti-pattern](https://www.richsnapp.com/blog/2019/06-09-reduce-spread-anti-pattern)
- [Software below the poverty line](https://staltz.com/software-below-the-poverty-line.html)

## Tips, tricks & bit-sized awesomeness:

- [Web Request and Declarative Net Request: Explaining the impact on Extensions in Manifest V3](https://blog.chromium.org/2019/06/web-request-and-declarative-net-request.html)
- [Vue RFC: Function-based Component API](https://github.com/vuejs/rfcs/pull/42)
- ["Why did this render?" a new React DevTools feature](https://twitter.com/brian_d_vaughn/status/1137522217728659456?s=09)
- [Array#{flat,flatMap} are available in Chrome, Firefox, Safari, and Node.js!](https://twitter.com/mathias/status/1138457106380709891?s=09)
- [Chrome Incognito mode is no longer detectable (as of Chrome 76)](https://twitter.com/paul_irish/status/1138471166115368960)
- [Apparently JavaScript is awesome (surprise) and as such it's possible to iterate ranges of numbers using the three dot operator](https://twitter.com/bmeurer/status/1138760699805601792)
- [Promise combinators](https://v8.dev/features/promise-combinators)
- [React Native Open Source Update June 2019](https://facebook.github.io/react-native/blog/2019/06/12/react-native-open-source-update)
- [Pica CDN: Introducing a CDN for modern JavaScript](https://www.pika.dev/cdn)
- [The state of GraphQL by Reddit](https://blog.graphqleditor.com/the-state-of-graphql-by-reddit/)
- [PR to Node.js to port rimraf to core](https://github.com/nodejs/node/pull/28208)

## Releases:

- [styled-components v5](https://twitter.com/mxstbr/status/1139541179421339648?s=09)
- [Stencil v1.0](https://ionicframework.com/blog/introducing-stencil-one-1-0-0/)
- [Atom 1.38](https://blog.atom.io/2019/06/11/atom-1-38.html)
- [React-redux v7.1.0](https://github.com/reduxjs/react-redux/releases/tag/v7.1.0)
- [VuePress v1.0](https://v1.vuepress.vuejs.org/)
- [React Native 0.60 RC1](https://github.com/facebook/react-native/releases/tag/v0.60.0-rc.1)
- [Webhint browser extension](https://twitter.com/webhintio/status/1139719760105467905?s=09)

## Libs & demos:

- [restated](https://github.com/mardix/restated)
- [threads.js](https://github.com/andywer/threads.js)
- [javascript-questions](https://github.com/lydiahallie/javascript-questions)
- [react-leaflet](https://react-leaflet.js.org/)
- [use-debounce](https://github.com/xnimorz/use-debounce)
- [react-resize-detector](https://github.com/maslianok/react-resize-detector)
- [react-teleporter](https://github.com/smooth-code/react-teleporter)
- [apple-auth](https://github.com/ananay/apple-auth)
- [passport-apple](https://github.com/ananay/passport-apple)
- [lebab](https://github.com/lebab/lebab)
- [readme-md-generator](https://github.com/kefranabg/readme-md-generator)
- [chessboardjs](https://github.com/oakmac/chessboardjs)
- [npmfs](https://npmfs.com/)

## Interesting & silly stuff:

- [GauGAN web app](http://3.16.206.30/)
